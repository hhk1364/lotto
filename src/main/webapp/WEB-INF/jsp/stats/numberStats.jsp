...
<title>번호별 통계</title>
<body>
 <form id="form" name="form" method="post" action="/stats/numberStats.do"> 
	   		...
	   		<table class="statsTable">
	   			...
	   			<tbody>
	   				<c:choose>
	   					<c:when test="${fn:length(result) > 0}">
	   						<c:forEach var="result" items="${result}" varStatus="status">
	   							<tr>
	   								...
	   								<td>
	   									<span class="num ball${result.num}"><c:out value="${result.num}"></c:out></span>
	   								</td>
	   								<td>
	   					...
	   					<c:otherwise>
	   						<tr>
	   							<td colspan="5">번호별 통계 데이터가 없습니다.</td>
	   						</tr>
	   					</c:otherwise>
	   				</c:choose>
	   			</tbody>
